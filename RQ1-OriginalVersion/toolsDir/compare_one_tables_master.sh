...
mariadb_command="mariadb -u${db_user} -p${db_pwd} ${db_name} -e"

get_version_sqlcommand="SELECT DISTINCT(system) FROM libcore_classes;"

tmp_libcore_systems_list="/tmp/temporary_libcore_systems_list"
$mariadb_command "${get_version_sqlcommand}" > "${tmp_libcore_systems}"
sed -i~ 1d "${tmp_libcore_systems_list}"


while read libcore_system; do 
    echo "______[${libcore_system}]"
    if [[ "${libcore_system}" =~ "master" ]]; then  
    	    #echo "______[${libcore_system}]"
		...
		if [[ ${nb_found} -eq 0 ]];then
		    ...
		    delete_libcore_row_sqlcommand="DELETE FROM libcore_8_classes WHERE system='${lc_system}' AND release_date='${lc_release}' AND libcore_path='${lc_path}' AND fuzzy_hash='${lc_digest}';"
